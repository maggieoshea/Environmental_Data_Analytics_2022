...
#What is its CRS again? WGS 1984 vs. counties uses NAD83
st_crs(huc8_sf)
st_crs(counties_sf)
...
windenergy_sf <- st_read('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
    colClasses = (AvgWindEnergy = numeric))%>%
...
merge_HUC8_wind <- merge( x= huc8_sf, 
                          y=windenergy_sf, 
                          by.x = 'HUC_8', 
                          by.y = 'HUC_8') 
...
intersect_HUCs <- huc8_sf_utm %>% 
  filter(st_intersects(x = ., y = triangle_sf, sparse = FALSE))

mapview(intersect_HUCs)
