...
> Answer: 
Null Hypothesis: AQI is greater than 50
Alternative Hypothesis: AQI is less than 50
...
> Ozone levels do meet the standard of *\mu/*, as we rejected the null that AQI is greater than 50.  
...
#Exercise 2:
#Ex 2.1
ex1_ozone.year <- ggplot(EPAair, aes(x = Ozone, fill  = as.factor(Year))) +
  geom_density(alpha = 0.5)+
  scale_fill_manual(values=c("darkgreen", "darkblue"))+
  labs(x="Ozone AQI Value", y="Density", fill="")
print(ex1_ozone.year)

#Ex 2.2
ex2.2 <- ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(y="Ozone AQI Value", x = "")+
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4,
               label = c("b", "a")) 
      #This last part says instead of the max value, but the label b and a

print(ex2.2)

#Ex 2.3
quantile(EPAair$Ozone, na.rm=TRUE)
ex2.3 <-
...
summary(PM.Ozone)
# R squared is 0.13 and the p-value on the coefficient for ozone is <0.001. The coefficient and associated p-value suggests a one unit increase in ozone is associated with a 0.38 increase in PM2.5. 
...
EPAvars <- 
  ...
  na.omit()
AirCorr <- cor(EPAvars)
corrplot(AirCorr, method = "ellipse")
corrplot.mixed(AirCorr, upper = "ellipse")
