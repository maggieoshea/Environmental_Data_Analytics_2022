...
# Because it is an integer and we are selecting only one year, the easiest code is filter. 



# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.1990to1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999))
...
NTL.phys.data.Tuesday1990to1999<-
  NTL.phys.data %>% 
...
NTL.irradiance.gathered <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "sample")

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.irradiance.spread <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)

#Is there a way to spread such that rather than column for each depth, you can group like depth is greater than 0.5 etc 
#Also is there a way to say get the names from depth and temperature from C and if row has NA under depth values can you make those 0 instead of resulting in NA in the code itself (of course can do it after)
```
