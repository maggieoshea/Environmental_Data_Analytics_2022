...
#Want the mean of the errors to be zero because if not, it means the regression is bias because it's always overestimating or underestimating --> of course it will fluctuate but want to have the mean at zero
```
...
#Divides console into 4 sections
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() +
  #scale y log makes the log the y axis
  geom_point() 
...
#Open this and it will tell you the correlation values
corrplot(PeterPaulCorr, method = "ellipse")
#Oval shapes represent scatterplot of one variable against the other, so the thinner the scatterplot the higher the approximation. The color is a strong but negative correlation (red) vs. dark blue is a strong positive correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
#Mix shows the corr value as well as the ellipses 
```
...
step(TPAIC)
#For each one it says when you remove variable (like po4 for ex), you get a different AIC --> higher AIC is worse 
