...
#data-to-viz.com --> decision tree plot for data visualization
...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=-0.025,y=0.025,label=expression(paste("Mean is ", mu,", ", R^{2},'=0.6')))
#geom_text is how you can put words on your plot. Note how you can also do super script
print(Plot_text)
...
EPAair2 <- read.csv("../Data/Processed/EPAair_O3_PM25_NC2122_Processed.csv", stringsAsFactors = TRUE)

EPAair2$Date <- as.Date(EPAair2$Date, format = "%Y-%m-%d")
...
#Notes: have to subset where depth = 0 because it's surface temperatures; ylab is changing the y axis name
surfacetemps <- ggplot(subset(PeterPaul.chem.nutrients, depth==0), aes(x = daynum, y = temperature_C)) + 
...
EPA_pivot<-pivot_wider(EPAair2, 
                              names_from = AQS_PARAMETER_DESC,
                              values_from = mean_AQI)

ggplot(EPA_pivot, aes(x = PM2.5, y = Ozone))+
