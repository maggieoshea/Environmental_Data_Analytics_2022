...
The pitfalls of using an absolute file path is that if you share the code with someone else they won't have the folder, and if you change the directory structure in any way you'll have to rewrite the paths 
...
> Answer: The console loads the packages and shows the steps in loading. For example, with tidyverse the console shows that all the internal packages that are within tidyverse with a check mark to show that these were loaded. 
...
#read.csv("/Users/maggieoshea/Desktop/Spring 2022/Data Analytics/Environmental_Data_Analytics_2022/Data/Raw/USGS_Site02085000_Flow_Raw.csv")
...
> ANSWER: This file was in the metadata folder. The placement in the metadata folder is helpful because it keeps the data separate from the metadata such that importing it does not require cleaning up the data to remove metadata information. The document itself is great for reproducability because it outlines important components of the data such as the meaning of key terms used, collection methods, and other choices made in designing the dataset. 
...
> Answer: The blank cells that were in a row with data in other columns are interpreted as "NA." 
...
> Answer: Under some analyses, such as time series where NA is in the middle of the time period, it is not best practice to remove NAs but for other methods it may be advantageous to remove the NAs and use the data that has been observed. 
...
```{r fixformartDate}
# 
USGS.flow.data.complete$datetime <- format(USGS.flow.data.complete$datetime, "%y%m%d")

#paste0 means glue characters/strings to specified input
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       })
#
USGS.flow.data.complete$datetime <- create.early.dates(USGS.flow.data.complete$datetime)

USGS.TEST <- USGS.flow.data.complete
#
USGS.TEST$datetime <- as.Date(USGS.TEST$datetime, format = "%Y/%m/%d") 
