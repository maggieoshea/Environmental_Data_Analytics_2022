...
* In the URL replace `02087500` with `02085070` and re-submit. What site's data do we have now? Eno River near Durham, NC... Saved a lot of clicks in getting to this dataset, didn't we? 
...
# sep = '\t' tells the site that the data is separated by tab (not comma, etc.) 
...
```{r exercise1pull.and.plot.discharge.data}
#Set the URL
...
```{r}
#Apply the API key
#census_api_key("2fef4e33b6b4476877e9407b30885a00f94e9a0a", install = TRUE)

#To view the variable codes
v17 <- load_variables(2017, "acs5", cache = TRUE)

maine.seasonalhousing <- get_acs(geography = "tract", 
                                 variables = "B25004_006",
                                 state = "ME", 
                                 geometry = TRUE)
maine.totalhousing <- get_acs(geography = "tract", 
                                 variables = "B25001_001",
                                 state = "ME", 
                                 geometry = TRUE)
library(sf)
maine.perchousing <- st_join(maine.seasonalhousing, maine.totalhousing, by = "NAME")


percseasonal.ME <- maine.seasonalhousing %>% 
  mutate("totalhousingunits" = B25081_008 + B25027_002)%>%
  mutate ("percseasonal" = (B25004_006/totalhousingunits)*100)


ggplot(maine.seasonalhousing, aes(fill = estimate, color = estimate)) +
  geom_sf() +
  coord_sf(crs = 4326) +
...
the_states <- '.basic_table a'
the_price <- 'td:nth-child(2)'
the_capacity <- 'td:nth-child(3)'
net_generation <- 'td:nth-child(4)'
total_retail <- 'td:nth-child(5)'

states_web <- the_website %>% html_nodes(the_states) %>% html_text()
price_web<- the_website %>%   html_nodes(the_price) %>%  html_text()
capacity_web <- the_website %>% html_nodes(the_capacity) %>% html_text()
generation_web <- the_website %>% html_nodes(net_generation) %>% html_text()
retail_web <- the_website %>% html_nodes(total_retail) %>% html_text()
  
#3 Construct a dataframe from the values
df_stateelectricity <- data.frame("State_Name" = as.character(states_web),
                             "Avg_retail_price" = as.numeric(price_web),
                             "NetSummerCapacity" = as.character(capacity_web), 
                             "NetGeneration" = as.character(generation_web), 
                             "TotalRetailSales" = as.character(retail_web))

#remove commas, make numeric
df_stateelectricity$NetSummerCapacity <- as.numeric(gsub(",","",df_stateelectricity$NetSummerCapacity))

df_stateelectricity$NetGeneration <- as.numeric(gsub(",","",df_stateelectricity$NetGeneration))

df_stateelectricity$TotalRetailSales <- as.numeric(gsub(",","",df_stateelectricity$TotalRetailSales))
...
scrape.it <- function(the_year){
  ...
  the_df <- data.frame("State_Name" = as.character(states_web),
                             "Avg_retail_price" = as.numeric(price_web),
                             "NetSummerCapacity" = as.numeric(gsub(",","", capacity_web)), 
                             "NetGeneration" = as.numeric(gsub(",","",generation_web)), 
                             "TotalRetailSales" = as.numeric(gsub(",","",retail_web)), 
                             "Year" = the_year)
...
electricity.years = rep(2017:2020)

#Use lapply to apply the scrape function
the_dfs <- lapply(X = electricity.years,
                  FUN = scrape.it)

fix_df <- bind_rows(the_dfs)



ggplot(fix_df,aes(y = Avg_retail_price, x=Year)) +
  geom_line()

v2_df<- map(electricity.years, scrape.it) %>% bind_rows 
v2_df
