...
#c Now you should have a column with Month. Use the paste0() function to paste month and year together and create a date object. --> paste0() pastes all the elements together that you list in the parantheses 
...
#Exercise 5: Check if there is any missing data for both temperature and rainfall series. --> None.

summary(BRA_complete$Temperature_C)
summary(BRA_complete$Rainfall)

#Exercise 6: Plot both series over time on separate plots. Add a trend line using geom_smooth(). Can you say something about the trend just by visualizing the data? --> Both appear to have increasing trends, though this seems to be a stronger trend in the temperature. 
...
#Choose frequency based on period: if your time component is months then freq = 12, if daily then freq= 365

temp_ts <- ts(BRA_complete$Temperature_C, start = c(1901,1), frequency = 12)
head(temp_ts, 10)

rain_ts <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)
head(rain_ts, 10)
...
#Look at environments to see what to put for [,1:3] --> if you only wanted seasonal you could do [,2] for example
...
ggplot(rain_Components) +
  ...
  ylab(expression("Average Rainfall"))

ggplot(temp_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "Orange") +
  ylab(expression("Average Temperature (Celsius)"))



ggplot(temp_Components) +
  geom_line(aes(y = seasonal, x = Date),  size = 0.25) +
  geom_line(aes(y = remainder, x = Date), color = "Blue") +
  geom_line(aes(y = trend, x = Date), color = "Orange") +
  geom_line(aes(y = Observed, x = Date), color = "Red") +
  ylab(expression("Average Temperature (Celsius)"))
...
# Exercise 11: Apply one of the trend detection test to the original data set. Remember that the data has seasonal component, which test can be applied in this case? The Seasonal Mann-Kendall 

f_month <- month(first(BRA_complete$Date))
f_year <- year(first(BRA_complete$Date))
BRAtemperature_ts <- ts(BRA_complete$Temperature_C,
                   start=c(f_year,f_month),
                   frequency=12) 

#decompose
BRAtemp_decomp <- stl(BRAtemperature_ts, s.window = "periodic")

smk.temperature <- Kendall::SeasonalMannKendall(BRAtemperature_ts)

# Inspect results
smk.temperature
summary(smk.temperature)

#Analyze Results: Results point to p-value less than 0.05 and the null hypothesis is that the data is stationary. So, with a p-value that is small we can reject the null and say that we do have a trend in temperature over time. 


#Rainfall
f_month <- month(first(BRA_complete$Date))
f_year <- year(first(BRA_complete$Date))
BRArainfall_ts <- ts(BRA_complete$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12) 

#decompose
BRArain_decomp <- stl(BRArainfall_ts,s.window = "periodic")

smk.rain <- Kendall::SeasonalMannKendall(BRArainfall_ts)
...
#Analyze Results: Results point to p-value greater than 0.05 and the null hypothesis is that the data is stationary. So, with a larger p-value, we do not have evidence to reject the null hypothesis suggesting that the data is stationary. 



# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9.

#Did initial ts objects remove seasonality with the: "s.window: periodic"?

noseason_rain <- mutate(rain_Components,
        NoSeason = trend+remainder)

noseason_rainfall_ts <- ts(noseason_rain$NoSeason,
                   start=c(f_year,f_month),
                   frequency=12) 

Kendall::MannKendall(noseason_rainfall_ts)



#P-value is large so we cannot reject the null hypothesis, suggesting that the data may be stationary. 

noseason_temp <- mutate(temp_Components,
        NoSeason = trend+remainder)

noseason_temp_ts <- ts(noseason_temp$NoSeason,
                   start=c(f_year,f_month),
                   frequency=12) 

Kendall::MannKendall(noseason_temp_ts)


#Result provides a small p-value, rejecting the null and providing evidence that the data is not stationary. 
...
> Answer: These findings, from both the seasonal mann kendall and the mann-kendall, suggest that temperature is not stationary but is changing over time. Rainfall, for both models, 
