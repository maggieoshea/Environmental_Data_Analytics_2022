...
> Answer: The data appears organized into type of data with all of the dataframes under "data", the vectors under "values", and functions under "Functions." 

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: When hitting tab, it shows you all the columns in the dataframe from which you can select. 
...
```{r}
hightemp.durham<-c(49, 57, 40, 41, 44, 37, 41, 51, 52, 53)
lowtemp.durham<- c(30, 30, 18, 26, 26, 18, 26, 26, 28, 40)
```
...
fahrenheittocelsius<-function(x)
  x-32/1.8
celsius.hightemp<-fahrenheittocelsius(hightemp.durham)
celsius.lowtemp<-fahrenheittocelsius(lowtemp.durham)
print(celsius.hightemp)
print(celsius.lowtemp)
```
...
Durhamtemperature.df <- data.frame("Fahrenheit Highs"=hightemp.durham,"Fahrenheit Lows"=lowtemp.durham, "Celsius Highs"=celsius.hightemp, "Celsius Lows"=celsius.lowtemp)
...
```{r}
fahrenheit.sd<-sd(Durhamtemperature.df$Fahrenheit.Highs)
summary(Durhamtemperature.df$Fahrenheit.Highs)
```
#To call the function just for a column, it is necessary to use the code dataframe$column to indicate which column the function should run on, whereas to just run it on the dataframe the $column is not necessary. 
