...
This is because this type of data can provide insight to ecosystems and health of the lakes?
...
library(lubridate)
NTL.phys.data <- read.csv("/Users/maggieoshea/Desktop/Spring 2022/Data Analytics/Environmental_Data_Analytics_2022/Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)
...
```
Work with rows: 
- Filter --> chooses row based on column values
- Slice --> chooses rows based on location 
- Arrane --> changes the order of rows
Work with columns: 
- Select --> changes if column is included
- rename --> reenames column 
- Mutate --> creates new column 
- Relocate --> changes order of columns
Work with Groups of Rows: 
- summarise --> collapses a group into a single row 

### Filter
Keys for data wrangling: 
- Comment your code
...
#!= Will remove the lakes you don't want, rather than selecting the ones you want 
...
#Using >= or <= makes sure the value you listed is included 
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)

#%in% is a value matching operator so it will look at the values within the range you give
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304))

# Exercise: 
# filter NTL.phys.data for the year 1999
# what code do you need to use, based on the class of the variable? I used filter, value == 
...
> Answer: Because that's not a column in the dataset, so you cannot filter by it. It also isn't good practice because if someone were to replicate your methods they would not know why you were doing it whereas if you filter by the dates, they know you want only observations within a set of dates. 
...
NTL.phys.data.temparrange <- arrange(NTL.phys.data, desc(temperature_C))
#East Long Lake has the highest temperatures at a depth of 0.5, in 1998. Broadly, the smaller depths had warmer temperatures. Hummingbird Lake also had a number of high temperature recordings. 
```
### Select
...
```{r}
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "/Users/maggieoshea/Desktop/Spring 2022/Data Analytics/Environmental_Data_Analytics_2022/Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv")
```
...
> Because wrangling code can be long and complicated and keeping it all in one gets easily messy. 
