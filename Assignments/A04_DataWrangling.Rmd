...
```{r, warning=FALSE}
#12(a,b)
summarize.airpollution<- summary.spread %>%
  group_by(Month, Site.Name, Year) %>%
...
#13
dim(summarize.airpollution)
```

14. Why did we use the function `drop_na` rather than `na.omit`?

> Answer: "Drop_na" drops the rows that have missing values and only keeps the rows that are complete with no values of NA when running the function, whereas na.omit removes them all together. 
